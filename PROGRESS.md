...
| 25 | UI Redesign v2 — Phases 1–8 (`design_tasks.md`) | ✅ Complete | 202 | 202/202 unchanged (frontend-only day) | 71 commits across eight phase branches, merged to `main` as PRs #37–#45. **Phase 1** restructured colour into primitive → semantic → component and mapped Tailwind's `--color-*` onto the semantic layer with `@theme inline` — load-bearing, because `inline` compiles utilities to `var(--sem-…)` rather than a frozen value, which is what lets **Phase 7** ship dark mode as one `[data-theme="dark"]` block with no utility or markup touched. Phase 1 was proven byte-identical on built output before anything was built on it. Phases: 1 tokens · 2 shell · 3 hero · 3b `/welcome` · 4 booking · 5 account · 6 organizer · 7 dark · 8 responsive/states/legal. **Bugs found and fixed:** ticket codes were being sent to `api.qrserver.com` in a query string (security); the organizer chart fabricated 30 daily buckets from real totals and labelled them as real; a genuine QR race hidden behind a cosmetic 2s timer; the Material Symbols icon font leaked glyph names into accessible names across 41 usages; three controls whose fill flipped with the theme while their label did not (`.btn-gradient` at 1.71:1 in dark); `--violet-400` referenced but never defined, so the hero pointer glow had never painted. **Verified:** 41 token pairs measured on built output in both themes (lowest dark 6.13:1); six routes swept live in both themes, zero failures; 375/768/1024/1440 as an organizer — no horizontal scroll, one `<main>`, no heading skips, no unnamed controls. Frontend build clean, vitest 4/4, lint 9 errors/6 warnings → **9/4**. |
| 26 | Booking Idempotency (make `Idempotency-Key` real) | 🟡 In Progress | 202 | — | Branch `feat/booking-idempotency`, cut from `main` at PR #45. `Idempotency-Key` is sent on `POST /api/v1/bookings` — which closed the Day 20 carryover as written and unblocks `app.rate-limit.enabled=true` — but it provides **no idempotency**: `RateLimitFilter` only checks the header is present (`RateLimitFilter.java:83`), never storing or comparing it, and the client mints a fresh `crypto.randomUUID()` per attempt so a retry carries a different key regardless. `disabled={isSubmitting}` already blocks the in-browser double-click, so exposure is narrow — a client timeout on a request that succeeded, two tabs, or an automatic network retry. That is the failure that charged booking 562 twice. Fix in two halves: persist the key with a UNIQUE constraint and return the existing booking on collision (the `processed_stripe_events` pattern from Fix 9.2 — next migration **V14__**), and mint the key once per reservation attempt client-side. TDD mandate applies: duplicate-key test Red first. |
...
| Fix 25-tokens | CRITICAL | 25 | ✅ | Three-layer token system with `@theme inline` mapping Tailwind's namespace onto the semantic layer — the prerequisite for a runtime theme swap. Proven invisible on built output before anything was built on it |
| Fix 25-dark | HIGH | 25 | ✅ | Dark mode as one `[data-theme="dark"]` block; pre-paint inline script so no flash; toggle persists and defaults to `prefers-color-scheme`. M3 `-fixed` roles deliberately not redeclared, keeping the ten status chips stable across themes |
| Fix 25-qrleak | SECURITY | 25 | ✅ | Ticket codes were being sent to `api.qrserver.com` in a query string — the venue-door credential handed to a third party. Removed from both ticket views in one commit so it was never live in an intermediate state |
| Fix 25-qrrace | HIGH | 25 | ✅ | Confirmation page fetched before RabbitMQ had written the QR codes; a cosmetic 2s shimmer hid that it never re-fetched. Real polling added (600ms / 12s ceiling / cancellation flag) |
| Fix 25-chart | HIGH | 25 | ✅ | `buildSalesSeries` fabricated 30 daily buckets from real totals with an invented weighting curve and presented them as real per-day figures. Replaced with per-event bars from real `grossRevenue`, verified against `SELECT sum(total_amount)` |
| Fix 25-iconfont | A11Y | 25 | ✅ | Material Symbols ligatures leaked glyph names into accessible names ("arrow_back Back to Home"). 41 usages / 11 files replaced with inline SVG; render-blocking font request removed; newly un-named icon controls given explicit `aria-label`s |
| Fix 25-contrast | HIGH | 25 | ✅ | `.btn-gradient` white-on-pale-violet at 1.71:1 in dark, plus `.btn-glass` and a "Total Paid" label at 1.36:1. ⚠️ Missed by the dark sweep because it reads `background-color` and a gradient is a `background-image` |
...
| Fix 26-idem | HIGH | 26 | ⬜ | `Idempotency-Key` sent but never stored or compared — `RateLimitFilter` checks presence only, and the client mints a fresh UUID per attempt. To close with UNIQUE constraint + `DataIntegrityViolationException` (the `processed_stripe_events` pattern), migration `V14__`, plus a stable client-side key |
