...
- Fix E-008 — SECURITY: Add Catch-All + IllegalState handlers to GlobalExceptionHandler
...
### Evening (1 hr) — Security Hardening (E-008) + Week 1 Checkpoint

#### Fix E-008 — GlobalExceptionHandler Security Patch

Add the following two methods to `GlobalExceptionHandler.java`. This takes < 10 minutes and closes a real information leakage gap before any production deployment.

**Method 1 — Catch-all for unexpected server errors (prevents stack trace leakage):**

```java
@ExceptionHandler(Exception.class)
public ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception ex) {
    log.error("Unexpected exception caught by global handler", ex); // ex as 2nd arg — mandatory for SLF4J stack trace
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(ApiResponse.failure("An unexpected error occurred. Please try again later."));
}
```

**Method 2 — Business rule violations (prevents 500 on booking errors):**
`BookingService.reserveTickets()` throws `IllegalStateException` for "Event not open" and "Not enough tickets". Without this handler they return HTTP 500 (wrong — misleads the client). They should return HTTP 422:
...
[ ] E-008: GlobalExceptionHandler has catch-all Exception handler (with ex arg)
[ ] E-008: GlobalExceptionHandler has IllegalStateException/IllegalArgumentException handler returning HTTP 422
```
